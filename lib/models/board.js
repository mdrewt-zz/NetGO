...
Board.prototype.locToCoords = function(location) {
  return {row: this.letterToNum(location[0]), column: parseInt(location.slice(1)) - 1};
};
...
Board.prototype.validateMove = function(location) {
    ...
    if(this.position[location]) { console.log("You can't go there."); return false; }
    ...
    if(spaces.empty.length < 1) { console.log("No liberties"); return false; }
    return true;
};
