...
Board.prototype.drawStone = function(x, y, color) {
  var stone = {type:"circle", x: x, y: y, color: color};
  this.renderer.drawCircle(this.calculatePath(stone));
};

Board.prototype.drawPosition = function(stones) {
  console.log("drawing: " + JSON.stringify(stones));
  for (var i=0; i<stones.length; i++) {
    this.drawStone(stones[i].row, stones[i].column, stones[i].player);
  }
};
