...
Renderer.prototype.drawStone = function(column, row, status) {
    ...
    if (status != "empty") {
        var stone = {
                        x: this.spacesToPixels(column), 
                        y: this.spacesToPixels(18 - row), 
...
Renderer.prototype.drawPosition = function(game) {
    console.log("Attempting to draw stone.")
    console.log(game.position)
    try {
        for(var x in game.position) {
            var stone = game.position[x];
            var coords = game.locToCoords(stone.location);
            console.log("Stone status is " + stone.status);
            console.log("Stone location is " + coords.toString)
            this.drawStone(coords.row, coords.column, stone.status);
        }
    } catch(err) {}
};